...
GitHub does not trigger workflow runs from events created using the automatic `GITHUB_TOKEN` — pushes and pull requests opened by `submission-open-pr.yml` itself do not start a new `ci.yml` run. The `npm test && npm run validate` step already inside `submission-open-pr.yml` is therefore the only automated gate these pull requests get; a human reviewer's own push or re-run can trigger `ci.yml` if a second check is wanted, but it does not happen automatically. Do not add a branch protection rule that requires the `ci.yml` check to pass before merge — since that check never runs on these PRs, such a rule would make every accepted submission unmergeable.
