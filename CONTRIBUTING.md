...
- `url_resolves` — a valid original public HTTP(S) source URL without embedded credentials is always required. With authorized text, source blocking or deletion does not prevent preservation; otherwise the URL is checked for resolution. Required repository validation enforces credential-free HTTP(S) source and archive links on manually edited records too.
