...
5. Record the model, evaluation date, hindsight version, prompt revision, and any adjudication. The `HINDSIGHT` block embedded in `review_prompt.md` is **v1** — frozen as the historical record of the July run; do not send it as-is to a new review. Assemble the current block per the "Assembling the block for a review" section of [`hindsight.md`](hindsight.md) and record the version used (currently `hindsight_version: v2`). New facts or corrections must go through [`ERRATA.md`](ERRATA.md) and produce a new hindsight version — never a silent replacement of the block — and `ERRATA.md` itself is never included in a reviewer's prompt.
